<script lang="ts">
    import { AnimatePresence, Box, Button, Card, Pill, cn } from "@repo/ui"

    const columns = 12
    const rows = 8

    const gridItems: { x: number, y: number }[] = []

    for (let i = 0; i < rows; i++) {
        for (let j = 0; j < columns; j++) {
            gridItems.push({ x: j + 1, y: i + 1 })
        }
    }
</script>
...
<Box tag="section" class="min-h-[calc(100vh-106px)] w-full items-center relative">
    <div class="grid grid-cols-[repeat(12,1fr)] grid-rows-[repeat(8,1fr)] overflow-hidden size-full">
        <div class="contents">
            {#each gridItems as { x, y }, index (index)}
                <div
                    style:--x={x}
                    style:--y={y}
                    class={cn(
                        "relative -z-10 col-start-[var(--x)] row-start-[var(--y)] hover:bg-muted",
                        x !== 12 && "border-r",
                        y !== 8 && "border-b",
                    )}
                />
        ...
        <div class="grid bg-background items-center mr-px mb-px gap-6 max-w-2xl p-8 col-start-2 row-start-2 col-span-5 row-span-6 z-20">
        ...
        <div class="bg-background grid items-center mb-px mr-px col-start-8 row-start-3 col-span-4 row-span-4 z-10">
            <div class="grid size-full items-center rounded-full relative">
                <Pill class="row-start-1 col-start-1 justify-self-center px-0 py-0 bg-gradient-to-br from-transparent to-secondary shadow-2xl">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-40 h-40 p-8">
...
<Box tag="section">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
        <Card class="grid grid-rows-[min-content_1fr_min-content]">
            ...
            <Card.Content>
                <Card.Description>
                    Our agile development process is built around 2-week sprints, allowing for rapid progress and frequent feedback.
